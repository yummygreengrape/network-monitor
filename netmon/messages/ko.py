...
# 실행된 발이 하나도 돌아오지 않았는데 실행 실패가 섞여 있으면, 몇 발이
# 실제로 나갔는지조차 모른다 — "일부" 라고 적을 근거가 없다(전부 실패했을
# 수도 있다). 손실률도 네트워크 손실로 읽을 수 없다.
FIRST_HOP_EVIDENCE_BURST_NOT_RUN = "첫 홉 증거는 동시 다발 ICMP — 응답이 하나도 없는데 실행되지 못한 측정이 섞여 있어 나간 발이 있었는지 알 수 없음."
