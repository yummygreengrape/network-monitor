...
# 사실을 함께 적는다** — 링크가 살아 있는 끊김과 섞이면 상위 원인이 따로 있는
# 것(링크)과 VPN 경로 자체의 문제가 한 덩어리가 된다. 그 주기에는 첫 홉도
# 리졸버도 재지 못하므로 원인을 더 좁히지 않는다.
VPN_DISCONNECTED_NO_LINK = ("%s 연결 끊김(공급자 상태 %s). 같은 주기에 주 인터페이스가 "
                            "없었음 — 링크가 없는 동안의 끊김이라 VPN 경로 자체의 문제로 "
                            "보지 않음. 이 주기는 다른 관측이 비어 있어 더 좁히지 않음.")
...
WZ_EXTERNAL_BODY = ("  고정 호스트의 TLS 발급자와 공인 IP 변화를 봅니다.\n"
                    ...
                    "  첫 주기에도 한 번 나가고, 공급자마다 한 끊김에 최대 12발까지만 보냅니다.\n"
