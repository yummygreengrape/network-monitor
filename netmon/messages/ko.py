...
# 공급자 상태가 connecting 인 전환. Warp 의 연결 단계나 Tailscale 의 Starting 처럼
# 다시 맺는 중인 상태가 여기로 옮겨진다.
# "터널" 이라고 적지 않는다 — 공급자가 보고하는 것은 connecting 이라는 상태뿐이다.
# (보호 상실 판정도 이 경우 그대로 나고 등급·조사 개시도 같다. 그 요약문의
# 머리말도 같은 기준으로 재협상이라 적는다 — VPN_PROTECTION_LOST_HEAD_RENEGOTIATING.)
VPN_RENEGOTIATING = "%s 연결 재협상 중(공급자 상태 connecting). 가장 유력한 설명: %s."
...
VPN_RENEGOTIATING_NO_LINK = ("%s 연결 재협상 중(공급자 상태 connecting). 같은 주기에 주 "
                             "인터페이스가 없었음 — 한 주기 안의 관측이라 링크 소실과 "
                             "재협상의 선후는 가리지 못하고, 이 판정은 공급자가 보고한 "
                             "상태 말고 다른 관측을 읽지 않아 원인을 좁히지 않음.")
# 여기 걸리는 모드는 DNS only 모드다(netmon/vpn/__init__.py 의 warp_tunnel_for) —
# 그 모드에서 공급자는 DNS 를 암호화해 받는다.
# 그래서 "이 공급자의 보호를 받지 못함" 이라고 적지 않는다. 말할 수 있는 것은
# 터널이 없다는 것(모드 이름)과 이 네트워크의 노출 정도(암호화 방식)까지다.
VPN_TUNNEL_OFF = ("%s 는 연결돼 있지만 터널을 세우지 않는 모드(%s). 이 네트워크는 같은 L2 의 "
                  "다른 기기가 들여다볼 수 있는 곳(트래픽 자체는 관측하지 않음).")
...
# 보호 상실 요약문은 머리말 + 본문이다(netmon/detect/vpn.py 의 `_protection_summary`).
# 머리말은 공급자가 보고한 상태를, 본문은 이 네트워크의 노출 정도를 말한다.
# connecting 이면 "끊김" 이라 적지 않고, 재협상 동안 보호가 이어지는지는 공급자
# 보고만으로 알 수 없으므로 그렇게 적는다(사용자 결정 2026-09-23).
VPN_PROTECTION_LOST_HEAD = "%s 끊김 — 이 공급자가 주던 보호가 끊긴 상태."
VPN_PROTECTION_LOST_HEAD_RENEGOTIATING = ("%s 연결 재협상 중(공급자 상태 connecting) — 그동안 "
                                          "이 공급자가 주던 보호가 이어지는지는 공급자 "
                                          "보고만으로 알 수 없음.")
VPN_PROTECTION_LOST = "이 네트워크는 같은 L2 의 다른 기기가 들여다볼 수 있는 곳(트래픽 자체는 관측하지 않음)."
VPN_PROTECTION_LOST_UNKNOWN = "이 네트워크를 신뢰할 수 있는지는 판단 불가."
...
# 보호 상실 요약문의 본문(머리말은 VPN_PROTECTION_LOST_HEAD*).
VPN_PROTECTION_LOST_SAE = ("WPA3-SAE 라 같은 L2 의 기기가 조용히 읽지는 못하지만, 들어온 "
                           "기기가 경로를 가로채면 그 위로 가는 것을 볼 수 있음(트래픽 "
                           "자체는 관측하지 않음).")
