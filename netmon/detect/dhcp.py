...
def detect(prev: Optional[Observation], cur: Observation, ctx) -> List[Finding]:
    ...
    if p_srv and c_srv and p_srv != c_srv:
        out.append(Finding(
            ...
            summary=("같은 네트워크에 머문 채로 DHCP 서버가 바뀌었다. "
                     "rogue DHCP 의 전형적 모양이다." if not attribution else
                     "DHCP 서버가 바뀌었으나 같은 시점에 네트워크가 달라졌다."),
    ...
    if p_r and c_r and p_r != c_r:
        out.append(Finding(
            ...
            confidence=CONFIRMED,
            severity=LOW if attribution else HIGH,
            summary="DHCP 가 알려주는 기본 라우터가 바뀌었다.",
    ...
    if p_d and c_d and p_d != c_d:
        out.append(Finding(
            ...
            confidence=CONFIRMED,
            severity=LOW if attribution else HIGH,
            summary="DHCP 가 알려주는 DNS 서버가 바뀌었다. 이름 해석을 가로채는 첫 단계일 수 있다.",
    ...
    if p_l and c_l and p_l != c_l:
        out.append(Finding(
            ...
            summary="DHCP 임대가 새로 시작됐다. 링크가 한 번 끊겼다가 붙었다는 뜻이다.",
