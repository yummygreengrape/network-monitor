...
재면 한 주기가 최소 2초 늘어납니다. 평소 주기에는 **한 번만** 쏘고, 손실은 주기
사이의 연속 실패로 봅니다. 이 변경으로 한 주기가 3.6초에서 2.3초가 됐습니다.
(끊김 언저리의 주기만 예외입니다 — 바로 아래 절.)
...
- **한 주기 늦게 시작됩니다**: 주소가 직전 주기의 관측에서 오므로 끊긴 **첫 주기에는
  보낼 주소가 없습니다.** 같은 이유로 다시 연결된 직후 첫 주기에도 나갈 수 있습니다 —
  그 주기의 판단 근거는 아직 끊겨 있던 직전 주기이기 때문입니다. 실제로 나가는지는
  그 직전 주기의 사유에 주소가 있었는지에 달렸습니다(이 절 아래의 표본 참조).
...
- **어디로**: 공급자가 사유 문자열에 적어 준 IPv4 주소 하나뿐입니다. 대상을 **외부
  문자열이 정하는** 자리라 공인 유니캐스트만 받습니다 — 막는 대역은
  `netmon/vpn/__init__.py` 의 `BLOCKED_V4` 에 열거돼 있고, 사설(10/8·172.16/12·
  192.168/16)·루프백·링크로컬·멀티캐스트 말고도 `0.0.0.0/8`, CGNAT(`100.64.0.0/10`),
  예약 대역(`240.0.0.0/4`, 브로드캐스트 주소가 여기 듭니다)이 함께 들어 있습니다.
  막힌 주소는 보내지도, 기록하지도 않습니다. IPv4 가 없으면(IPv6 만 있으면) 재지
  않습니다. 공급자가 여럿이면 이름 순으로 훑되 **주소를 내주지 못한 공급자는 건너뛰고**,
  주소를 내준 공급자 중 첫 번째 하나를 씁니다.

**"공급자가 알려 준 상대편" 은 늘 있는 값이 아닙니다.** 이 기계의 보관 표본 7일치
(2026-09-16~22, **2026-09-22 19시(한국 시각) 시점** 관측 86,225주기 — 수집이 계속 돌므로 나중에
세면 더 큽니다)를 세어 보면, 공급자 사유에 주소가 들어 있던 주기는 한 공급자(WARP)의
`connecting` 주기 86건뿐이었습니다. 같은 기간의 비연결 주기는 모두 930건
(그 공급자의 `connecting` 539 + `disconnected` 328, 다른 공급자의 `disconnected` 63)
이었고, **`connecting` 밖에서는 한 건도 나오지 않았습니다.** 다만 그 539주기 가운데
414주기는 사유가 Settings Changed 였고, 이것은 끊김 상태(`Disconnected`)의 사유로
보입니다 — warp-cli 에 `DisconnectedReason::SettingsChanged` 라는 형 이름이 있습니다.
상태 해석은 "disconnected" 에도 "connect" 가 들어 있는 탓에 `Disconnected` 를
`connecting` 으로 옮기고 있었고, 이 결함은 고쳤습니다(`netmon/vpn/__init__.py` 의
`Warp.status`). 표본은 상태 원문을 저장하지 않고 WARP 데몬 로그는 2026-09-22T14:40Z
이후만 남아 있어, 그 주기들이 실제로 무엇이었는지는 확정하지 못합니다. 사유가 Manual
Disconnection 인 7주기는 판단하지 못합니다(근거는 `tests/test_detect_vpn.py` 의
`TestWarpStatusParsing`). 주소가 나온 86건은 전부 연결 단계 사유가 붙은 나머지
118주기에서 나왔습니다. 여기서 "비연결" 은 보내는
두 상태(`disconnected`·`connecting`)만 말합니다 — 조회 실패(`unknown`) 주기는 세지
않았습니다(그 상태에는 애초에 보내지 않습니다). 나온 주소는 1종이었습니다. 다른 망에서
어떤지는 재지 않았습니다.
