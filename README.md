...
| `external_probes` | DNS 가로채기·TLS 발급자·공인 IP 확인, VPN 이 끊긴 주기의 터널 상대편 도달성 | 고정된 조회 이름과 내 출발지 IP. 터널 엔드포인트 측정을 켜면, VPN 이 연결돼 있지 않은 주기에 한해 **공급자가 사유 문자열에 적어 준 주소**(공인 유니캐스트만)로 ICMP 한 발 |
